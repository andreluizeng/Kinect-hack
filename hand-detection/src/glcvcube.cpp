...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeCreate (float w, float h, float d)
{
	int i = 0;
	int j = 0;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeTranslate (float x, float z, float y)
{
 	float trans[16] = {0};
	float aux[16] = {0};
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeMove	(int axis, float value)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeRotate (int axis, float angle)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeScale (int axis, float value)
{
	float aux[16] = {0};
	float scale[16] = {0};
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeLoadIdentity (void)
{
	LoadIdentityMatrix4x4 (_modelview_matrix);	
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetFaceTex (int face, IplImage *texture_data)
{
	switch (face)
	{
		case FRONT_FACE:	cvCvtColor (texture_data, _texture_data[FRONT_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[FRONT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[FRONT_FACE]->imageData);
								break;

		case BACK_FACE:	cvCvtColor (texture_data, _texture_data[BACK_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[BACK_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[BACK_FACE]->imageData);
								break;

		case LEFT_FACE:	cvCvtColor (texture_data, _texture_data[LEFT_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[LEFT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[LEFT_FACE]->imageData);
								break;

		case RIGHT_FACE:	cvCvtColor (texture_data, _texture_data[RIGHT_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[RIGHT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[RIGHT_FACE]->imageData);
								break;

		case TOP_FACE:		cvCvtColor (texture_data, _texture_data[TOP_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[TOP_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[TOP_FACE]->imageData);
								break;

		case BOTTOM_FACE:	cvCvtColor (texture_data, _texture_data[BOTTOM_FACE], CV_BGR2RGB);
								glBindTexture(GL_TEXTURE_2D, _texture[BOTTOM_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[BOTTOM_FACE]->imageData);
								break;
...
// Desc: Texturize a determined cube face with the given texture
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetFaceTexBuf (int face, char *texture_data, int w, int h)
{
	switch (face)
	{
...
//--------------------------------------------------------------------------------------
// Name: CubeSetFaceColor()
// Desc: Paint a determined face with the giver color
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetFaceColor (int face, CvScalar color)
{
	switch (face)
	{
		case FRONT_FACE:	cvSet (_texture_data[FRONT_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[FRONT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[FRONT_FACE]->imageData);
								break;

		case BACK_FACE:	cvSet (_texture_data[BACK_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[BACK_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[BACK_FACE]->imageData);
								break;

		case LEFT_FACE:	cvSet (_texture_data[LEFT_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[LEFT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[LEFT_FACE]->imageData);
								
								break;

		case RIGHT_FACE:	cvSet (_texture_data[RIGHT_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[RIGHT_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[RIGHT_FACE]->imageData);
								break;

		case TOP_FACE:		cvSet (_texture_data[TOP_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[TOP_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[TOP_FACE]->imageData);
								break;

		case BOTTOM_FACE:	cvSet (_texture_data[BOTTOM_FACE], color, NULL);
								glBindTexture(GL_TEXTURE_2D, _texture[BOTTOM_FACE]);
								glTexImage2D (GL_TEXTURE_2D, 0, GL_RGB, _texture_w, _texture_h, 0, GL_RGB, GL_UNSIGNED_BYTE, _texture_data[BOTTOM_FACE]->imageData);
								break;
...
//--------------------------------------------------------------------------------------
float GLCVCube::CubeGetAxisAngle (int axis)
{
	float angle = 0;
...
//--------------------------------------------------------------------------------------
float GLCVCube::CubeGetAxisPos (int axis)
{
	float pos = 0;
...
void GLCVCube::CubeShowCoords (void)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeShowUVCoords (void)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeShowModelMatrix (void)
{
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVCube::MultMatrix4x4 (float *matC, float *matA, float *matB)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::LoadIdentityMatrix4x4 (float *m)
{
	m[0] = 1;
	m[1] = 0;
	m[2] = 0;
...
//--------------------------------------------------------------------------------------
GLuint GLCVCube::GenTextures (void)
{
	GLuint texture_id = 0;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeCreateTex	(int texture_w, int texture_h)
{
	// textures
	_texture_w = texture_w;
	_texture_h = texture_h;

	_texture_data[FRONT_FACE] 	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
	_texture_data[BACK_FACE] 	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
	_texture_data[LEFT_FACE] 	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
	_texture_data[RIGHT_FACE] 	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
	_texture_data[TOP_FACE] 	= cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
	_texture_data[BOTTOM_FACE] = cvCreateImage (cvSize (_texture_w, _texture_h), 8, 3);
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeDestroyTex (void)
{
	cvReleaseImage (&_texture_data[FRONT_FACE]);
	cvReleaseImage (&_texture_data[BACK_FACE]);
	cvReleaseImage (&_texture_data[LEFT_FACE]);
	cvReleaseImage (&_texture_data[RIGHT_FACE]);
	cvReleaseImage (&_texture_data[TOP_FACE]);
	cvReleaseImage (&_texture_data[BOTTOM_FACE]);
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetProjMatrix (float *projection_matrix)
{
...
//--------------------------------------------------------------------------------------
bool GLCVCube::CreateShaderProgram (void)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetModelMatrix (float *m)
{
	int i;
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetRotationMatrix (float *m)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeGetModelViewMatrix(float*m)
{
	for (int i = 0; i < 16; i++)
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeSetTranslationMatrix (float *m)
{
...
//--------------------------------------------------------------------------------------
void GLCVCube::CubeDestroy (void)
{
	int i;
	...
	for (i = 0; i < 6; i ++)
	{
		...
		cvReleaseImage (&_texture_data[i]);
	}
