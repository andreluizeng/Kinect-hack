...
void Render (void)
{
	...
	*/
	...
	plane_video1->PlaneMove(PLANE_Z_AXIS, -1500);
	...
	plane_video1->PlaneSetTexBin(gray_depth);
...
void depth_cb(freenect_device *dev, void *v_depth, uint32_t timestamp)
{
	...
	for (i = 0; i < 640 * 480; i++)
	{
		gray_depth->imageData[i] = 255-ceil ((0.124 * t_gamma[depth[i]]) + 0.876);
		//gray_depth->imageData[i] = t_gamma[depth[i]];
		//printf ("\ndepth mid: %d", t_gamma[depth[i]]);		
	}
	/*
	for (i=0; i<640*480; i++) {
